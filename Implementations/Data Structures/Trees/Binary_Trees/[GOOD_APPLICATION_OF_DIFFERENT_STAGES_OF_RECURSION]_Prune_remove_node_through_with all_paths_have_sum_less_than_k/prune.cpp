...
Node* prune_tree(Node* root, int& k, int &sum){
    if(!root)
        return NULL;
    int l_sum = sum + root->data;
    int r_sum = l_sum;
    root->left = prune_tree(root->left, k, l_sum);
    root->right = prune_tree(root->right, k, r_sum);
    sum = max(l_sum, r_sum);
    if(sum < k){
        delete root;
        root = NULL;
...
Node* prune_tree_util(Node* root, int& k){
    int sum = 0;
    return prune_tree(root, k, sum);
}
